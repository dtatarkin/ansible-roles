...
**Helm values extraction:** HelmChart/HelmChartConfig templates separate the `valuesContent` into a dedicated `*-values.yml.j2` file. The helmchart template references it with:
```yaml
  valuesContent: |-
    {{ lookup('template', '<role>-values.yml.j2') | indent(4) }}
```
This keeps helmchart structure and values concerns separate. When creating new K3s roles or modifying existing ones, follow this pattern.
